...
| Superpowers | Use explicit skills for systematic debugging, verification before completion, TDD, and worktree isolation. | Superpowers remains the execution surface for its official skills. OPL Flow routes to it and keeps the default profile lighter than full always-on Superpowers. |
