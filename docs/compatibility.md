...
| Ponytail | Use YAGNI, stdlib/native-first implementation, and over-engineering review as a tactical simplification lens. | Ponytail is optional. Keep its default `off` or `lite`, invoke it explicitly for simplification or `ponytail-review`, and do not let it override OPL Flow evidence, ops, verifier, or completion-audit rules. |
