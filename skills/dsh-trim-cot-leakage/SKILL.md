...
description: Use when auditing or fixing developer prose that exposes authoring-session reasoning, dead draft references, PR or review choreography, change narration, control-flow walkthroughs, or unowned planning residue instead of repository-resolvable facts.
---

# Trim Authoring-Session Leakage

Use `$opl-doc` and `$dsh-prose-standard`. The target is prose whose viewpoint is
