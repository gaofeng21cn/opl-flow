...
- Git 写任务用独立 worktree/branch 与写集唯一 owner；创建即用 `worktree_lifecycle.py register` 登记，旁路 receipt 不算 owner。owner 负责 checkpoint、fresh-main 重放/解冲突、验证、main/wire 回读和自有 worktree/ref/临时产物清理；handoff 仅在接收方明确接管后生效。无 owner 先 recovery，仅在 clean、holder=0、吸收与远端 parity 通过后清理；`.worktrees` 只放 Git worktree。
