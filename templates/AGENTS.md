...
- 同一目标只设一个主控；ACTIVE 必须有 live execution owner 和可立即执行的 `next_action`。仅在子任务独立、可验收且缩短关键路径时单层委派；单个主控任务内的子智能体并发默认 4，分批证明有益后可到 8，超过 8 须用户明确授权。子智能体不得再委派，完成或阻塞时立即回报并收拢，主控同轮续派。
- Git 写任务使用独立 worktree 和分支，按精确写集设唯一 owner；仅在真实依赖、写集重叠或 `main`/发布集成时串行；吸收前同步远端 `main` 并按 SSOT 语义重放，吸收后验证最终 `main` 与远端一致并立即清理本任务临时 Git 表面。
