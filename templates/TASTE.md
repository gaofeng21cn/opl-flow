...
9. **隔离并行**
   ...
   Git 写入任务默认在任务自有 worktree 和分支中推进，根仓 `main` 只承担短时集成、最终验证和发布。写入 owner 应按精确写集划分，而不是把整个仓库长期锁给一个任务；同仓非重叠工作可以并行，重叠写集和 `main` 吸收窗口必须显式交接。吸收前以最新 canonical `main`、远端 currentness 和当前 SSOT 为准解决冲突，不能让旧基线覆盖新主线；吸收后验证最终 `main` 字节并清理任务自有临时 Git 表面。
