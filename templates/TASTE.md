...
对 domain / product / delivery 任务，真实进度按目标交付物、owner decision、human gate、可接力 artifact delta 或用户可用结果计算；runtime、queue、stage、read model、测试、docs、provider liveness 和平台修复只是支持证据。OPL/MAS/MAG/RCA 等执行基座是首选路径，不是目标交付的前置条件：基座顺畅时走基座，基座卡住时，前端执行者继续推进合法的目标交付增量，并把暴露出的基座问题作为 side repair lane 记录或修复。只有继续动作会越权写 authority surface、缺少必要内容/数据源、或同一写集 ownership 冲突且无法隔离时，才暂停具体目标动作。repair lane 不能吞掉目标主线，除非用户目标本身就是平台修复。
...
13. **风险分层优先于测试仪式**
    ...
    测试、TDD、集成验证、端到端验证、live readback、runtime receipt、日志、截图和人工检查都是证据工具；选择依据是风险、claim 类型和反馈成本，而不是固定开发仪式。默认先按 risk-based development flow 选择风险档和验证预算：普通小改动优先 focused evidence，高风险 contract / permission / data write 才新增高杠杆测试，runtime / currentness / release / readiness 结论优先 live artifact、read model、owner receipt 或端到端 acceptance。TDD 只在用户明确要求、稳定低成本 bug 回归、durable contract/API/schema/authority 边界或接口设计需要 test-first 澄清时使用；低价值、重复、实现细节绑定、文案绑定、旧兼容面和 flaky 测试应删除、合并或避免新增。

14. **本因诊断优先于状态复述**

    长时间停滞、反复失败、监控告警或自动推进循环出现时，不能只复述表层状态。必须把现象追溯到可改变的本因：是目标产物本身仍不满足要求，是 gate / evaluator 设计或 currentness 误判，是 owner route / authority / handoff 流程设计缺口，还是 runtime / control-plane 基座缺陷。有效结论应说明“为什么卡住”、哪个 owner 能修、下一步用什么合法入口修、如何证明修后真的产生可接力进展。状态检查只有在转化为 blocker-to-owner map、修复计划、human gate 或明确停止条件时才算完成。
