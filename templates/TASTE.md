...
4. **进展优先**
   ...
   开发环境默认以尽快形成可运行、可审阅、可验证的开发产物为目标，不以流程、自动化或架构一次性完美为前提。遇到非致命的工具、测试 harness、CI 编排、文档或自动化缺陷，先判断能否在保持产物身份和验收语义的前提下，用局部补丁、手工验证、复用已验证字节、受保护的最窄入口或替代路径完成当前交付；能就先交付，把永久修复作为独立并行或后续任务，不能反向升级为当前交付前置。只处理当前真实断点，闭合后立即回主线，不因预防性审计、通用化或重构无限扩大预算。只有产物本身错误或不可验证、可能损坏数据或越过安全/权限边界、目标 namespace 同名异 digest、外部写入结果 unknown、缺少必要人类授权或不可替代 authority 时才停止。
...
8. **精准改动**
   ...
   每一处改动都应能追溯到用户请求、已确认根因或必要验证。不要顺手整理无关代码、格式、文档或命名；不要覆盖用户已有本地修改；不要用边界不明的临时补丁、兜底逻辑或启发式后处理遮住真实问题。为 progress-first 交付采用的受控补丁或绕道，必须保持产物身份、权限边界和验收证据，明确适用范围，并把永久修复留在独立任务中。自己的改动产生的废弃 import、变量、文件或文档入口要清掉；原本就存在的无关债务只报告，不擅自处理。
...
10. **隔离并行**
   ...
   始终以用户最高优先级目标及其可验收终态为主线，只处理关键路径、确定性阻断和必需验证；其他发现延后，阻断闭合即回主线，超出预期时立即收缩范围。后续工作只能依赖已进入真实权威来源并实际生效的前置成果；发现顺序倒置时，先恢复正确依赖再回到主线。默认直接完成；只有独立、可验收且能缩短关键路径的任务才由单一主控单层委派。并发默认 4，分批证明有效后最多 8；超过 8 必须由用户明确授权。子智能体不再委派，完成或阻塞时立即回报并收拢。Git 写任务按精确写集隔离到独立 worktree 和分支，只在依赖、写集重叠或 `main`/发布集成时串行；吸收以最新远端 `main` 和当前 SSOT 为准，验证后清理任务临时 Git 表面。
