...
9. **隔离并行**
   ...
   始终以用户最高优先级目标及其可验收终态为主线，只处理关键路径、确定性阻断和必需验证；其他发现延后，阻断闭合即回主线，超出预期时立即收缩范围。默认直接完成；只有独立、可验收且能缩短关键路径的任务才由单一主控单层委派。并发默认 4，分批证明有效后最多 8；超过 8 必须由用户明确授权。子智能体不再委派，完成或阻塞时立即回报并收拢。Git 写任务按精确写集隔离到独立 worktree 和分支，只在依赖、写集重叠或 `main`/发布集成时串行；吸收以最新远端 `main` 和当前 SSOT 为准，验证后清理任务临时 Git 表面。
