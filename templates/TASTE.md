...
Machine boundary: 本文是人读协作偏好；项目事实、接口约束、验收结论和机器真相以用户指令、源码、contracts、docs、runtime 输出、owner receipt 和 repo-native 验证为准。

## 读法

`TASTE.md` 不是项目架构文档，也不是 OPL 运行规则。它定义 AI 怎么干活：如何理解目标、选择路径、控制改动、使用证据、交付结果。

OPL Flow 默认把本文安装并维护在用户级 `~/.codex/TASTE.md`。具体项目怎么开发、怎么运行、谁持有真相、哪些命令算验收，应放在各仓 `AGENTS.md`、docs、contracts、source、tests、runtime/readback 和 owner surface 中，不通过 repo-local `TASTE.md` 分叉维护。
...
OPL Flow 安装器负责把本文同步到用户级 `~/.codex/TASTE.md`。项目如需局部规则，应在本仓 `AGENTS.md`、`docs/decisions.md`、`docs/invariants.md`、contract、source 或更深层规范写清适用范围。OPL 怎么开发、各 domain agent 怎么运行、App 怎么验收，都属于项目规则，不属于用户级 `TASTE.md`。
